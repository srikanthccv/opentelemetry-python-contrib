...
docker run --rm \
  ...
  -v ${ROOT_DIR}/util/opentelemetry-util-semconv/src/opentelemetry/util/semconv/trace/:/output \
...
docker run --rm \
  ...
  -v ${SCRIPT_DIR}/templates:/templates \
  -v ${ROOT_DIR}/util/opentelemetry-util-semconv/src/opentelemetry/util/semconv/resources/:/output \
  otel/semconvgen:0.2.1 \
  -f /source code \
  --template /templates/semantic_attributes.j2 \
  --output /output/__init__.py \
